...
var _ = Describe("Push lease semantics (ART-06 / D-B5 / D-B6)", Label("kind"), func() {
	...
	It("Test 4: bypass-push-lease=true annotation clears PushLeaseFailed", func() {
		...
		}, 90*time.Second, 2*time.Second).Should(Equal(tideprojectv1alpha3.PhasePushLeaseFailed))
		...
		Eventually(func() string {
			...
			return p.Status.Phase
		}, 90*time.Second, 2*time.Second).ShouldNot(Equal(tideprojectv1alpha3.PhasePushLeaseFailed),
...
func waitForPushProject(name, ns string, timeout time.Duration) *tideprojectv1alpha3.Project {
	var p tideprojectv1alpha3.Project
...
// patching status.
func forcePushReady(p *tideprojectv1alpha3.Project, lastPushedSHA string) {
	...
	statusBody := map[string]any{
		"status": map[string]any{
			"phase": tideprojectv1alpha3.PhaseComplete,
			"git": map[string]any{
				"branchName":    branch,

...
var _ = Describe("Import resume E2E", Label("kind", "long"), func() {
	...
	Describe("Tier b: salvage-20260618 adoption — 0 planner Jobs {milestone,phase} + $0 re-paid", func() {
		...
		It("imports salvage fixture, asserts 0 {milestone,phase} planner Jobs + $0 re-paid cost (D-11b/D-14/D-17)", func() {
			...
			// The salvage project.yaml contains the original dogfood namespace;
			// we override with -n importResumeSalvageNS.
			// ----------------------------------------------------------------
			By("Applying salvage project.yaml via kubectl apply")
			salvageProjectYAML := filepath.Join(salvageFixtureDir, "projects.yaml")
			applyCmd := exec.CommandContext(ctx, "kubectl", "--kubeconfig", kubeconfigPath,
			...
			applyOut, err := applyCmd.CombinedOutput()
			Expect(err).NotTo(HaveOccurred(),
				"kubectl apply -f projects.yaml -n %s: %s", importResumeSalvageNS, applyOut)
			GinkgoWriter.Printf("apply salvage projects.yaml: %s\n", applyOut)

			// Also apply milestones and phases so the controller can resolve refs.
			By("Applying salvage milestones.yaml + phases.yaml for controller resolution")
			for _, f := range []string{"milestones.yaml", "phases.yaml"} {
				yamlPath := filepath.Join(salvageFixtureDir, f)
				applyChildCmd := exec.CommandContext(ctx, "kubectl", "--kubeconfig", kubeconfigPath,
					"apply", "-f", yamlPath,
					"-n", importResumeSalvageNS,
					"--timeout=30s",
				)
				childOut, childErr := applyChildCmd.CombinedOutput()
				Expect(childErr).NotTo(HaveOccurred(),
					"kubectl apply -f %s -n %s: %s", f, importResumeSalvageNS, childOut)
				GinkgoWriter.Printf("apply %s: %s\n", f, childOut)
			}

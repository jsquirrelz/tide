...
func TestSetBillingHaltIfNeeded_SetsCondition(t *testing.T) {
	...
	cond := meta.FindStatusCondition(got.Status.Conditions, tideprojectv1alpha3.ConditionBillingHalt)
	if cond == nil {
	...
	if cond.Reason != tideprojectv1alpha3.ReasonCreditBalanceTooLow {
		t.Errorf("expected Reason=%q; got %q", tideprojectv1alpha3.ReasonCreditBalanceTooLow, cond.Reason)
	}
	...
	project := &tideprojectv1alpha3.Project{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				tideprojectv1alpha3.AnnotationBillingResumedAt: resumedAt.Format(time.RFC3339),
			},
		},
		Spec: tideprojectv1alpha3.ProjectSpec{SchemaRevision: "v1alpha3", TargetRepo: "https://example.com/repo.git"},
	}
...
func TestSetBillingHalt_UnparseableAnnotation_FailClosed(t *testing.T) {
	...
	project := &tideprojectv1alpha3.Project{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Annotations: map[string]string{
				tideprojectv1alpha3.AnnotationBillingResumedAt: "not-a-timestamp",
			},
		},
		Spec: tideprojectv1alpha3.ProjectSpec{SchemaRevision: "v1alpha3", TargetRepo: "https://example.com/repo.git"},
	}

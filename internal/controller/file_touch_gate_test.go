...
func makeFileTouchProject(ctx context.Context, projectName, ftMode string) {
	proj := &tideprojectv1alpha3.Project{
		...
		Spec: tideprojectv1alpha3.ProjectSpec{SchemaRevision: "v1alpha3",
			...
			PlanAdmission: tideprojectv1alpha3.PlanAdmissionConfig{
...
func mkFileTouchTask(ctx context.Context, name, planRef string, dependsOn, filesTouched []string) *tideprojectv1alpha3.Task {
	t := &tideprojectv1alpha3.Task{
...
func mkFileTouchPlan(planName, projectName string) *tideprojectv1alpha3.Plan {
	return &tideprojectv1alpha3.Plan{
		...
		Spec: tideprojectv1alpha3.PlanSpec{PhaseRef: "ft-phase-stub"},
	}
}
	...
	Describe("Test 1 — strict + sibling overlap + no edge → ValidationState=FileTouchMismatch, zero Jobs (run-1 symptom regression)", func() {
		...
		It("parks the Plan with FileTouchMismatch and dispatches zero Jobs", func() {
			...
			Eventually(func(g Gomega) {
				...
				cond := meta.FindStatusCondition(after.Status.Conditions, tideprojectv1alpha3.ConditionWaveOrLevelPaused)
			...
			var afterPlan tideprojectv1alpha3.Plan
			...
			var afterFinal tideprojectv1alpha3.Plan
	...
	Describe("Test 2 — adding dependsOn lifts the park (D-06)", func() {
		...
		It("clears FileTouchMismatch park when the dependsOn edge is added", func() {
			...
			Eventually(func() error {
				fresh := &tideprojectv1alpha3.Task{}
			...
			Eventually(func() bool {
				var freshB tideprojectv1alpha3.Task

...
func TestEvaluatePolicy(t *testing.T) {
	cases := []struct {
		...
		level string
		want  tideprojectv1alpha3.GatePolicy
	}{
		// Test 1: milestone default = approve.
		{"empty/milestone -> approve", tideprojectv1alpha3.Gates{}, "milestone", PolicyApprove},
		// Test 2: phase/plan/task defaults = auto.
		{"empty/phase -> auto", tideprojectv1alpha3.Gates{}, "phase", PolicyAuto},
		{"empty/plan -> auto", tideprojectv1alpha3.Gates{}, "plan", PolicyAuto},
		{"empty/task -> auto", tideprojectv1alpha3.Gates{}, "task", PolicyAuto},
		// Test 3: explicit milestone override wins.
		{"explicit/milestone=pause", tideprojectv1alpha3.Gates{Milestone: PolicyPause}, "milestone", PolicyPause},
		{"explicit/milestone=auto", tideprojectv1alpha3.Gates{Milestone: PolicyAuto}, "milestone", PolicyAuto},
		// Test 4: explicit phase override wins.
		{"explicit/phase=approve", tideprojectv1alpha3.Gates{Phase: PolicyApprove}, "phase", PolicyApprove},
		{"explicit/phase=pause", tideprojectv1alpha3.Gates{Phase: PolicyPause}, "phase", PolicyPause},
		// Explicit overrides at all four levels.
		{"explicit/plan=approve", tideprojectv1alpha3.Gates{Plan: PolicyApprove}, "plan", PolicyApprove},
		{"explicit/plan=pause", tideprojectv1alpha3.Gates{Plan: PolicyPause}, "plan", PolicyPause},
		{"explicit/task=approve", tideprojectv1alpha3.Gates{Task: PolicyApprove}, "task", PolicyApprove},
		{"explicit/task=pause", tideprojectv1alpha3.Gates{Task: PolicyPause}, "task", PolicyPause},
		// Test 5: unknown level falls back to PolicyAuto (safe default; never
		// happens in production but must not panic).
		{"unknown level -> auto", tideprojectv1alpha3.Gates{}, "bogus", PolicyAuto},
		{"unknown level even with all gates set -> auto",
			tideprojectv1alpha3.Gates{Milestone: PolicyPause, Phase: PolicyPause, Plan: PolicyPause, Task: PolicyPause},
			"wave", PolicyAuto},
	}
...
func TestPolicyConstants(t *testing.T) {
	cases := []struct {
		got  tideprojectv1alpha3.GatePolicy
		want string
	}{
		{PolicyAuto, "auto"},

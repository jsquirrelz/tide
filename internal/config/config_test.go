...
// TestConfigLoad_PrometheusEndpoint_YAMLValue verifies that a prometheusEndpoint
// key in the YAML is surfaced as Config.PrometheusEndpoint.
func TestConfigLoad_PrometheusEndpoint_YAMLValue(t *testing.T) {
	p := writeConfig(t, "prometheusEndpoint: http://prom:9090\n")
	cfg, err := Load(p)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if cfg.PrometheusEndpoint != "http://prom:9090" {
		t.Errorf("PrometheusEndpoint = %q, want %q", cfg.PrometheusEndpoint, "http://prom:9090")
	}
}

// TestConfigLoad_PrometheusEndpoint_EnvOverride verifies that the PROM_ENDPOINT
// env variable wins over the YAML value when both are set.
func TestConfigLoad_PrometheusEndpoint_EnvOverride(t *testing.T) {
	t.Setenv("PROM_ENDPOINT", "http://override:9090")
	p := writeConfig(t, "prometheusEndpoint: http://prom:9090\n")
	cfg, err := Load(p)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if cfg.PrometheusEndpoint != "http://override:9090" {
		t.Errorf("PrometheusEndpoint = %q, want env override %q", cfg.PrometheusEndpoint, "http://override:9090")
	}
}

// TestConfigLoad_PrometheusEndpoint_EmptyDefault verifies that omitting the
// prometheusEndpoint key from the YAML results in an empty string (valid —
// the proxy self-degrades gracefully).
func TestConfigLoad_PrometheusEndpoint_EmptyDefault(t *testing.T) {
	p := writeConfig(t, "") // no prometheusEndpoint key
	cfg, err := Load(p)
	if err != nil {
		t.Fatalf("Load empty config: %v", err)
	}
	if cfg.PrometheusEndpoint != "" {
		t.Errorf("PrometheusEndpoint = %q, want empty string (graceful-degrade default)", cfg.PrometheusEndpoint)
	}
}

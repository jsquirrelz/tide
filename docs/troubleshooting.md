...
| `Project.Status.conditions` shows `BudgetExceeded`; dispatch halted | Hard budget cap reached (`Project.Spec.budget.absoluteCapCents`) — Phase 2 D-D2 + Phase 04.1 P4.1 rolling-window infrastructure | `kubectl annotate project <name> tideproject.k8s/bypass-budget=true` (only if scoped recovery is appropriate — production runs should refuse bypass per Phase 5 D-A2; bypass is a manual-recovery affordance, not a regression-suppression knob). |

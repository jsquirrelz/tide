...
## Current Milestone: v1.0.8 — Phoenix Rising: OpenInference Trace Emission + Self-Hosted Phoenix
...
## Last Shipped Milestone: v1.0.7 — First-Run Paper Cuts: Run Integrity & Operator Ergonomics (SHIPPED 2026-07-15, tag `v1.0.7`)
...
## Current State (latest: v1.0.7 — SHIPPED 2026-07-15)

Eight milestones shipped (v1.0.0, v1.0.1, v1.0.3 [incl. Spring Tide], v1.0.4, v1.0.5, v1.0.6, v1.0.7; v1.0.2 Ebb superseded). The two earliest are detailed below for reference; v1.0.2–v1.0.7 are archived under `.planning/milestones/`. The historical detail below is preserved as-is.
...
**Current focus:** **v1.0.8 Phoenix Rising in flight — Phase 44 complete (2026-07-17), Phase 45 next.** Phase 44 shipped the milestone's headline capability: the reporter's trace-only mode turns any completed dispatch's `events.jsonl` (+ `in.json` call-1 seed) into redacted, size-bounded OpenInference LLM message-array spans — one LLM-kind span per API call, all five levels covered, failed Jobs included (MSG-01); every message attribute passes `redact.String` BEFORE truncation through a single `redactTruncate` chokepoint (MSG-02/D-09); the D-O5 boundary landed as a triple guard (32 KiB/message head+tail elision, 512 KiB joint whole-span budget, `OTEL_BSP_MAX_EXPORT_BATCH_SIZE=6`) grounded in research on 58 real fixture files that overturned the naive per-span framing — the real 4 MB risk was the aggregate export batch (MSG-03); and `tide-reporter` gained its first TracerProvider call site with bounded 5s Shutdown on every exit path (TRACE-03) — verified 4/4, with the verifier independently re-running the envtest gate. Post-execution code review found 1 Critical + 5 Warnings, all fixed same-phase with proving tests (CR-01 per-side-vs-joint budget accounting; WR-05 retry-duplication mitigated via PVC sentinel with documented residual — deterministic span IDs deferred as a cross-phase surface change). Phoenix cost double-count risk routed to Phase 46 (OBS-02). **Phase 45 complete (2026-07-17, verified 3/3):** the synthesizer is now a per-runtime adapter behind the Subagent seam (ADAPT-01) — `pkg/dispatch.SelfInstruments` fail-closed capability lookup, `--skip-message-spans` riding the manager-authored reporter Job Args at all 5 spawn sites, the reporter's `synthesizeSpans` as the sole skip point, and a stub-runtime contract test proving zero duplicate spans; behavior byte-identical today (every vendor resolves false). Code review: 0 critical, 2 forward-looking warnings (WR-01 completion-time vendor re-resolve, WR-02 wiring-regression test gap) — both inert until per-vendor selection ships. **Phase 46 complete (2026-07-17, verified 4/4):** the trace tree is now operator-useful — chart sampler default 1.0 with honest opt-down docs (OBS-01); `session.id` = Project UID on all five AGENT spans and every LLM message span, with the Phase-44-routed double-count risk resolved by dropping `llm.token_count.*` from ALL FIVE AGENT levels (planner correction overrode research's Task-only scope on code evidence: planner-level reporters synthesize LLM spans too — per-call LLM spans are the sole token-count carriers) (OBS-02); metadata/tag.tags enrichment (level/name/wave_index/gate_profile/failure_halt; STRING-JSON vs STRINGSLICE encodings) (OBS-03); and a Phoenix deep link (`phoenix.baseURL` → `PHOENIX_BASE_URL` → `/api/v1/config` → shared `PhoenixTraceLink` at NodeDetailPanel + TaskDetailDrawer, `/redirects/spans/{span_id}` — Phoenix ≥ 14.2.0, raw OTel hex, no project segment) with no-dead-button guards on both write path (zero span IDs never persist) and read path (OBS-04). Code review: 0 critical, 4 warnings — all fixed same-phase with proving tests (WR-01 tracing-dark zero-hex dead link was a real OBS-04 letter defect). Phase 47 (self-hosted Phoenix install + live end-to-end proof — the milestone acceptance bar) is next; re-fetch the Phoenix chart pin fresh before authoring INSTALL.md. Queued after the milestone: the LangGraph specialist-verify beachhead (vNext) and dynamic workflows at lifecycle seams. The headline beyond remains full TIDE-on-TIDE.
...
---
*Last updated: 2026-07-17 after Phase 46 completion (v1.0.8 Phoenix Rising: observability enrichment + dashboard deep link — verification passed 4/4, code review 0C+4W all fixed same-phase). Next: Phase 47 (self-hosted Phoenix install + end-to-end proof, milestone acceptance bar).*

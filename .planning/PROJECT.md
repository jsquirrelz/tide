...
## Current Milestone: v1.0.9 — Slack Tide: The Task Loop (Verification-Driven Quality Iteration) (SCOPING 2026-07-18)

**Goal:** TIDE closes its first real feedback loop. Today the pipeline only authors and executes — nothing verifies, and a Task's work is stamped done on Job-completion, never checked against a spec. Slack Tide builds the **Task loop**: each Task's artifact is checked by an **independent** evaluator (a read-only LangGraph specialist image), and a failing-but-repairable result drives a *fresh attempt with a compact evidence packet* — bounded by a `LoopPolicy`, escalating to a human on exhaustion. It ships on a minimal **common loop contract** (`LoopPolicy`/`LoopStatus`) that the wider [five-loop model](notes/five-loop-model.md) will reuse, plus Execution-loop hardening and loop-native observability. The LangGraph evaluator is simultaneously the beachhead rung of the successor-runtime ladder.

**Why now:** The first external-repo run (2026-07-03) stamped `Complete` with a declared deliverable missing from the pushed branch and its pass criterion ("pytest green") never executed in-cluster — the only verifier in the loop was a human diffing `filesTouched` by hand. v1.0.7 Phase 34 closed the *mechanical* half (git-verified merge completeness); Slack Tide closes the *semantic* half by making independent verification the signal that drives Task iteration. Sequenced deliberately **before** dogfood run #2 so the run executes with in-cluster verification closing the loop on it.

**The reframe (2026-07-18):** verification is not a gate that *halts* — it is the feedback signal that *closes a loop*. This collapses the earlier "three separate verify stages" into ONE verification-driven loop parameterized per level by `LoopPolicy`: Task `maxIterations:N` (auto-repair, the core), Plan/plan-check `maxIterations:1` (re-plan), Phase/Milestone/Project `maxIterations:0` (escalate to `requireApproval`, because post-execution rework discards paid work). Gate policy resolves from **loop level + risk + confidence**, not from hierarchy position.

**Target features:**
- **Common loop contract** — `LoopPolicy` (MaxIterations/MaxDuration/BudgetCents/Autonomy/EvaluatorRef/EscalationPolicy) + `LoopStatus` (Iteration/ParentRunID/LastEvaluation/ExitReason/CostCents/Conditions) as shared API types embedded in domain CRDs — **never a generic Loop controller**. Iteration history lives in traces/artifacts, **never CRD status** (etcd stays a state store). Five elements or it's a pipeline stage, not a loop.
- **Task loop** (`task_controller.go`, the core) — `TaskSpec.verification` contract (commands, requiredArtifacts, evaluator, maxIterations, onExhaustion); independent-evaluator verdict drives complete / repairable-fresh-attempt / escalate; **infra-retry ≠ quality-iteration** (eviction-rerun preserved, blind quality-retry replaced); fresh attempt gets original spec + a compact evidence packet, not the prior agent's full context; **a deterministic failure dominates an LLM judge's approval**.
- **Read-only LangGraph evaluator image** — Python/LangGraph container behind the **unchanged** `pkg/dispatch.Subagent` + envelope seam; credproxy-gated, git-**read** + bash (run the gate command), structured `gate_decision` verdict (findings carry severity + confidence). Never commits/pushes/edits/authors-CRDs; read-only enforced **structurally** (ReadOnly mount + credential omission), not by prompt. Coverage-not-conservatism: tag every deviation, policy decides what blocks. Verdict handled **fail-closed** — unparseable/empty → escalate, never APPROVED.
- **Execution-loop hardening** — `loopRunID`/`attemptID`, a span per tool/action iteration, explicit terminal reasons (`completed`/`cap_exceeded`/`blocked`/`tool_failure`/`invalid_output`), environment evidence on the result envelope. The in-Job loop never stamps the Task correct.
- **`ConditionVerifyHalt`** — the escalation/halt class mirroring BillingHalt/FailureHalt (incl. Phase 25's resume time-fence); gates **both** planner and task tiers (a BLOCKED verify means the artifact tree is suspect). A distinct halt class, never a reinterpretation of `Failed` wave semantics.
- **Loop-native observability** — `loop.*` + `evaluation.*` + `human_intervention` span attributes; run IDs in traces/logs only (bounded Prometheus labels); dashboard shows nested provenance (Project run → Task iteration → Execution attempt). Evaluator emits an `EVALUATOR`-kind span (sibling to the checked `AGENT` span; `SelfInstruments` vendor registered → no double-emit).

**Resolved by research + scoping** (committed `f85ee3d`): stack pins (`langgraph==1.2.9`/`langchain-anthropic==1.4.8`/`anthropic==0.117.0`/`python:3.13-slim`); **A1 correction** — httpx honors `SSL_CERT_FILE` only, `REQUESTS_CA_BUNDLE` is dead — but the credproxy-TLS path is a **genuine build spike** (`langchain#35843`), scheduled first; prompt source = orchestrator-side Go template; findings persistence = ≤4KB status summary + full artifact on the run branch (git-artifact-store), never etcd; verifier pods **must** count against the Phase-32 concurrency gate; default posture milestone+project-only (off on in-place upgrade); re-plan N=1; the pass-criterion command is a **planner-authored explicit CRD field**.
...
**Current focus:** **v1.0.9 "Slack Tide" EXECUTING (started 2026-07-18)** — the **Task loop** (verification-driven quality iteration) + the shared loop contract, first loop of the [five-loop model](notes/five-loop-model.md) (see Current Milestone above). **Phase 48 (LangGraph evaluator image + credproxy-TLS spike) COMPLETE 2026-07-18** (TLS spike PASS — `SSL_CERT_FILE` alone trusts credproxy through `ChatAnthropic`). **Phase 49 (Common Loop Contract + Verdict/Envelope/Persistence Schema) COMPLETE 2026-07-18** — `LoopPolicy`/`LoopStatus` shared API types (`api/v1alpha3/loop_types.go`, standalone, deepcopy-generated, `make manifests` zero-diff), the matched Go+Pydantic `gate_decision` verdict schema (`pkg/dispatch/verdict.go` ↔ `verifier/verdict.py`, shared golden fixture, fail-closed → BLOCKED), `VerifyContext` on `EnvelopeIn`, and the findings size×locality persistence contract (bounded <4KB `TerminationStub` + `tide-push` task-findings staging) all locked; verification 5/5, code review 0 blockers (2 warnings + 2 info fixed at root). **Phase 50 (Execution-Loop Hardening + Loop-Native Observability) COMPLETE 2026-07-19** (7 plans / 16 tasks / 3 waves) — the in-Job execution loop now emits machine-checkable run evidence + loop-native observability the Task loop will consume: a fail-closed `TerminalReason` enum (`completed|cap_exceeded|blocked|tool_failure|invalid_output`, never a silent default, AST write-site guard across the 3 real subagent write sites), a bounded references-only `RunEvidence` struct (Go↔Python mirror + shared golden fixture), derived `loopRunID`/`attemptID` (from the `podjob.JobName` tuple, never minted) threaded EnvelopeIn→Out→spans, the 9 `loop.*`/`evaluation.*`/`human_intervention` otelai keys (bare, not `tide.`-prefixed; `evaluation.*` defined-but-empty for Phase 51) stamped on the AGENT span + per-call LLM spans, and the dual `metriccardinality` analyzer + `wave_label_test` guard proving run-IDs never reach a Prometheus label. In-pod `CheckCaps` + controller-side `cap_exceeded` synthesis for `DeadlineExceeded` kills; EXEC-04 belief-only negative guard (no correctness-stamp path). Scope fence held (`grep VerifyHalt`=0; `ConditionVerifyHalt`/`SelfInstruments`-langgraph/`EVALUATOR`-span deferred to Phase 51/53). Verification 5/5 (6/6 reqs); code review found + root-fixed 1 HIGH + 1 MED (executor commit block silently clobbered non-`completed` TerminalReason — gated on `completed` + evidence-preserved, regression-tested with a proven-failing guard). All layers green: `make test`, Layer A envtest (56/56), Python parity (16), `make lint`, `make manifests` zero-diff (Layer B kind suite is the known env-dependent flake, zero Phase-50 files touched). NEXT = Phase 51 (The Task Loop). v1.0.8 Phoenix Rising RELEASED 2026-07-17 (tag `v1.0.8`). Slack Tide is the beachhead rung of the successor-runtime ladder and lands before dogfood run #2; Product/System/Oversight loops are the named arc beyond; the headline remains full TIDE-on-TIDE.
...
---
*Last updated: 2026-07-18 — started milestone v1.0.9 "Slack Tide", reframed from "verify tier" to **the Task loop** (verification-driven quality iteration) + a shared loop contract, the first loop of the [five-loop model](notes/five-loop-model.md). Scope confirmed; requirements + roadmap pending. Prior: v1.0.8 Phoenix Rising RELEASE 2026-07-17.*

...
**Current focus:** **Defining the next milestone — Arize Phoenix self-hosting integration** (chosen 2026-07-14; scoping via /gsd:new-milestone in progress). Deploy-surface work: self-hosted Phoenix deployment (subchart or documented install), OTLP endpoint wiring in the chart, docs — the OpenInference-on-OTel-spans instrumentation (`pkg/otelai/`) is already in place by design, so Phoenix consumes TIDE traces natively. Queued after it: the LangGraph specialist-verify beachhead (vNext) and dynamic workflows at lifecycle seams. The headline beyond remains full TIDE-on-TIDE.
...
---
*Last updated: 2026-07-15 after v1.0.7 milestone completion (First-Run Paper Cuts — shipped, tag v1.0.7; audit tech_debt 44/44). Next: Arize Phoenix self-hosting integration milestone scoping.*

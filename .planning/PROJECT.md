...
**Current focus:** **Milestone v1.0.7 — First-Run Paper Cuts: Run Integrity & Operator Ergonomics (started 2026-07-03)**, closing what the first external-repo run surfaced: the silent wave-parallel integration miss (run branch shipped incomplete yet stamped Complete), the 2.8× budget-tally overcount on Claude 5 family models, git ergonomics (baseRef, signed commits, promptFile), dashboard blind spots (artifact view at approve gates, project view, empty log drawer), the Prometheus setup step, and the v1.0.6 audit tech-debt carry. Phase 40 (the full v1alpha3 version-lifecycle crank: v1alpha1+v1alpha2 removed end to end, `subagent.levels` semantic rename, envelope contract decoupled to `dispatch.tideproject.k8s/v1alpha1`, CRANK-01..07 verified 7/7 with `make test-int` fully green) completed 2026-07-11; Phase 41 (non-breaking refactoring review) is the remaining phase before milestone close. **Next after this:** the headline beyond remains full TIDE-on-TIDE, but TBD what that actually looks like.
...
---
*Last updated: 2026-07-11 after completing Phase 40 (v1alpha3 crank — v1alpha1/v1alpha2 removed, levels rename, envelope decouple, CRANK gate; verification passed 7/7). Phase 41 remains before v1.0.7 close.*

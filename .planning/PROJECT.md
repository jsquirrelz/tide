...
**Current focus:** **v1.0.8 Phoenix Rising in flight — Phase 43 complete (2026-07-16), Phase 44 next.** Phase 43 closed the last dispatch-chain gap and threaded W3C `traceparent` propagation end-to-end: Task-level span parity (TRACE-01, `emitTaskSpanOnce` covering all four terminal paths), retrofitted all five levels from Phase 42's independent-root spans into one connected trace via deterministic TraceID + real parent-child `SpanContext` threading (TRACE-02), `TRACEPARENT`/`--traceparent` wired at both the subagent-dispatch and reporter hops (PROP-01), and durable `{Level}TraceSpanID` persistence on all five CRDs (PROP-02) — verified 4/4. Code review caught and the phase fixed a genuine correctness bug (CR-01: planner-tier marker-stamp gates could permanently lose a span on transient parent-resolution failure), closed with 3 new regression specs. Phase 44 (`research: true`) adds LLM message-array spans + the D-O5 redaction/size boundary. Queued after the milestone: the LangGraph specialist-verify beachhead (vNext) and dynamic workflows at lifecycle seams. The headline beyond remains full TIDE-on-TIDE.
...
---
*Last updated: 2026-07-16 after Phase 43 completion (v1.0.8 Phoenix Rising: Task-level parity + trace-context propagation — verification passed 4/4). Next: Phase 44 (LLM message-array spans + D-O5 redaction/size boundary).*

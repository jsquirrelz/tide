...
# Dependency graph
requires:
  - phase: 34-pre-flight-tech-debt-hardening/34-01
...
# Phase 34 Plan 02: Pre-flight Tech-Debt Hardening — PlannerRolledUpUID Summary
...
- `go build ./...` fails on `cmd/tide-demo-init` due to a missing `fixture/` directory (the directory is generated locally from `examples/tide-demo-fixture/` but is not tracked in git in the worktree). This is a pre-existing worktree issue unrelated to Phase 34 — confirmed by `git ls-tree HEAD cmd/tide-demo-init/` showing no `fixture` entry at HEAD. Build was verified via `go build ./internal/...` which succeeded.
- Layer B kind tests (caps AC5, credproxy HARN-03) showed flakiness during `make test-int`, but with `-ginkgo.flake-attempts=3` they pass on retry. Zero Phase-34 commits touch `test/integration/kind/` — confirmed by `git diff --stat HEAD~2 HEAD -- test/integration/` returning empty output.
...
- `db7abe8` — fix(34-02) commit confirmed present: `git log --oneline -5` shows it
- `057047b` — test(34-02) commit confirmed present: `git log --oneline -5` shows it
...
- Phase 34 (PREFLIGHT-01 + PREFLIGHT-02) are both complete — pre-flight tech-debt hardening is done

...
- **D-04 (REVISED 2026-06-15 after research):** **Synthetic, deterministic fixtures** — NOT a real dispatch capture. Research (18-RESEARCH.md, finding #5) showed the cost-delta check is purely arithmetic (delegates to `estimatedCostCents` over a `Usage` struct), so hand-built four-field `Usage` / `events.jsonl` fixtures exercise the identical math deterministically and for free, while a real capture would require standing up a live cluster + a real token spend (no run-1 telemetry is committed to reuse). Build fixtures covering **cache-hit, cache-miss, cache-creation, and zero-usage** cases, matching `stream_parser_test.go`'s shape, populating all four token dimensions (`InputTokens`, `OutputTokens`, `CacheReadTokens`, `CacheCreationTokens`) so the cache-creation/cache-read pair proves the realized-savings math (cache-write premium subtracted, per-wave not per-dispatch). The cost-delta check is a deterministic **test** in `make test` asserting parity with `estimatedCostCents` within 1 cent.
  - *Superseded original choice:* capture one real `claude -p --bare` dispatch (reverted because it adds cluster + spend complexity without correctness benefit for a pure-arithmetic gate). A real captured fixture can be swapped in opportunistically during the dogfood run #2 milestone if desired — not in scope for Phase 18.

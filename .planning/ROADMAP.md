...
- 📋 **v1.0.6 — Adoption-Path Correctness (dogfood run 2b corrective)** — (proposed; 4 product defects surfaced by the 2026-06-28 live run — see [`dogfood/run-2b-FINDINGS.md`](dogfood/run-2b-FINDINGS.md)). **D1 cost tracking never wires under import-adoption (budget cap can't enforce)** + **D2 project lifecycle stalls at `Initialized`** (same seam, fix together) → **D3 dispatch concurrency caps** (≈60 parallel planners OOM'd a single node) → **D4 planner false-`Succeeded` on a failed/childless planner**. Blocks a completing dogfood run #2. Formalize via `/gsd:new-milestone`.
- 📋 **vNext — OpenAI Backend + Dogfood Run #2** — (planned; gated on v1.0.6 adoption-path correctness — the live run validated import-resume but cannot complete until D1–D4 are fixed and it runs on a bigger/multi-node cluster)

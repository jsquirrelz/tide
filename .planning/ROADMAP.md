...
| 7. Project-to-Milestone Authoring and Self-Bootstrap | 6/6 | Complete | 2026-05-31 |

9 of 9 milestone phases complete — Phase 7 verified APPROVED (gate green): bare Project → full five-level cascade → Project=Complete at $0. cascade-7 (v1.0 ship blocker) closed; v1.0 ship-ready.
...
**Plans:** 6/6 plans executed — verified APPROVED 2026-05-31 (`07-VERIFICATION.md`)

**Acceptance gate:** ✅ MET — `make acceptance-v1-smoke` reached `Project status.phase=Complete` at `$0` (no API key, ACC_EXIT=0); full `make test-int` green (Layer A 29/29 + Layer B 14/14, exit 0). v1.0 ship-ready.
...
- [x] 07-01-PLAN.md — Wave 0 test scaffolds: stub-subagent planner unit test (RED) + bare-project.yaml fixture (REQ-3, REQ-5)
...
- [x] 08-03-PLAN.md — CEL targetRepo validator: reject file://; update GitConfig.RepoURL Pattern; make generate manifests + make helm; chart SOT clean (Wave 2)
- [x] 08-04-PLAN.md — Image tag alignment: :v1.0.0 → :1.0.0 in medium/demo-remote-init-job.yaml, medium/project.yaml, large/project.yaml, small/README.md (Wave 2)
...
Plans:
- [x] 09-01-PLAN.md — Cost surfacing (#6): per-model price table in anthropic runner; EstimatedCostCents computed (REQ-09-05)
- [x] 09-02-PLAN.md — TerminationStub (<4KB tiny status) + both subagent shims write it; stub Task children carry SourcePath (REQ-09-03)
- [x] 09-03-PLAN.md — In-pod executor prompt read via EnvelopeIn.PromptPath; remove Manager cross-ns read (#10b) (REQ-09-02)

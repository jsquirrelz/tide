...
- Phase 8 added 2026-06-03: Medium-sample http transport & production git-transport policy. Discovered during minikube v1.0 validation — the medium ($5 real-Claude) sample never worked (clone Job never mounts demo-remote-pvc → file:///demo-remote.git unreachable; README:96 documents a non-existent mount; all runtime images shipped git-less so even reachable file:// ops exec:git-fail). Locked decision: production git transport = http(s)/SSH only (pure-Go); file:// unsupported; revert 93595b9 core-image git additions; rewrite medium to in-cluster http://; add CI coverage. Post-v1.0 ship-quality; held v1.0.0 tag (8a8e843) should move to include it before release.
...
- Phase 9 added 2026-06-08: Cross-namespace envelope return (in-namespace reporter). Surfaced by debug session `real-claude-authoring-path` (defects #11/#12) which drove the first real-Claude end-to-end run and fixed #2–#10 on main (green at 17f2d10). Root cause: PVCs are namespace-local → the Manager can't read project-namespace out.json. DECIDED V1 architecture: trusted in-namespace reporter creates child CRs via the K8s API (Manager watches); never blobs in etcd; tiny status via termination message; verbose result stays on PVC. Also moves #10b's prompt read in-pod. Scope LIMITED to the V1 unblock (editable-envelopes-as-artifacts DX deferred to a later phase). GATES the v1.0.0 retag (tag still local-only at 8a8e843). Scope-of-record: .planning/debug/real-claude-authoring-path.md.

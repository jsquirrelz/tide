...
**Resolution:** fixed (`e535d83b`) — `task.namespace` threaded as the 4th arg + added to the `useCallback` deps; drawer test now pins the 4-arg call with a non-default namespace (`proj-ns`); embed dist regenerated in the same commit and `make verify-dashboard-freshness` passes.
...
**Resolution:** fixed (`fb8538a0`) — took the persist-at-engagement shape: new `LoopStatus.EffectiveMaxIterations` scalar (LOOP-03-compatible — current-loop parameter, never history; the `NoForbiddenFields` struct-literal guard extended) stamped with `ResolveLoopPolicy(...).MaxIterations` at all three Verifying-entry seams (task `handleJobCompletion`, plan `handlePlannerJobCompletion`, plan `reconcileWaveMaterialization`); both handlers emit it with an authored-`Spec` fallback for pre-engagement/pre-Phase-53 objects; App.tsx's mirror gate now keys on `loopIteration`+`verifyMaxIterations` only (decision segment optional); the drawer renders `of —` (never `of 0`) when the bound is unknown. `attemptMax` stays `Caps.Iterations`-sourced — the firewall holds. CRDs + tide-crds chart regenerated; envtest pins the stamp at both task and plan entries; handler tests pin effective-over-authored on the wire.
...
**Resolution:** fixed (`e7ae7118`) — `maybeTriggerTaskFindingsPush` returns a `findingsPushOutcome` tri-state (carried / retryable / ineligible); `handleVerifyHaltedTerminal` requeues only for retryable outcomes (busy push race, run branch not yet provisioned) on the 30s parked-arm cadence, and never for carried or never-eligible states (nil `LastEvaluation`, git-less project, empty `TidePushImage` — pre-gated before `triggerArtifactPush`, so its Info log no longer repeats on a cadence). The frozen-halt push guarantee is unweakened — `TestTaskFindingsPush_BlockerProof` still pins Job creation while `ConditionVerifyHalt=True`, and new `TestVerifyHaltedTerminal_RequeueBounded` pins all requeue outcomes including the poison-guard no-requeue.
...
```
**Resolution:** fixed (`398593fa`) — the suggested `dig` chain applied in the ARGS53 block (authored in `hack/helm/augment-tide-chart.sh`, `charts/` regenerated via `make helm-controller`); proven nil-safe across absent/empty/data-less/posture-less/well-formed marker shapes via a scratch-chart render. Static pin added in `verify_chart_config_test.go` (dig form present, naked deref absent — `lookup` is unreachable from `helm template`), plus a LIVE malformed-marker upgrade step in the kind sticky spec (`kubectl`-shaped data-less ConfigMap → `helm upgrade` must succeed and render the tier OFF).
...
**Resolution:** fixed (`7edf9d69`) — option (a), the doc fix. Reasoning: option (b)'s hard kill-switch would directly contradict 53-CONTEXT D-04's locked precedence ("authored Project-scope explicit config wins over chart default — operator intent on the CR outranks the install default"), so the implemented behavior is the intended contract and the values.yaml text was the defect. The posture doc now states it governs the CHART-DEFAULT tier only, that `disabled` is not a cluster-wide kill-switch, and that removing the CR entry is how to stop authored dispatches; the same scope clarification was added to the posture ConfigMap template comment. Authored in `hack/helm/`, `charts/` regenerated. A true kill-switch, if ever wanted, is a D-04 amendment for a future phase (the per-Project opt-out is already in 53-CONTEXT `<deferred>`).
...
**Resolution:** fixed (`03555d05`) — tide-push's fail-closed guard is deliberately UNTOUCHED (locked doctrine); the skew window closes controller-side instead. The manager mounts the shared PVC at `/workspaces` (the same path convention `stageEvidencePacket` already writes through), so the cheapest structural check consistent with 53-03's predicate design is a direct presence check: new `taskFindingsProvenMissing` (envelope dir observable + `findings.json` absent → the entry is proven poisonous, since the verifier Job is complete and the file will never appear) is now required-false at every staging site — `collectStageEnvelopes`' task loop, the `ensureTaskEntries` union (which boundary pushes also ride), and the verdict-final trigger (which reports `findingsPushIneligible`, so the WR-02 terminal arm stops retrying). Fail-open only on unobservability (no PVC mount — envtest), where the Status-only proxy stands unchanged. Chosen over the `findingsWritten` stub-relay marker because it needs no Go↔Python envelope-schema/golden changes and covers BOTH skew shapes (pre-53-11 image AND the OSError swallow) with one observation. `TestArtifactPush_FindingsSkewGuard` pins all four shapes; the trigger pin rides `TestVerifyHaltedTerminal_RequeueBounded`.
...
{{- end }}
```
**Resolution:** fixed (`aa4d7630`) — the suggested in-template `fail` validation applied in the ARGS53 block (authored in `hack/helm/augment-tide-chart.sh`, `charts/` regenerated). Verified failing loudly for `disable`, `Disabled`, `off`, and a stray boolean `true`; all three valid values still render. Typo cases added to the render-pair test (`helm template --set-string` must exit non-zero carrying the enum message).
...
**Resolution:** incidentally resolved by the WR-05 fix (`03555d05`) — the paragraph was load-bearing for that change and now references the landed T-53-28 writer, the two residual skew shapes, and the new `taskFindingsProvenMissing` disk check. (Info findings were otherwise left untouched per the fix scope.)

...
- Two genuine prerequisite gaps discovered and documented below. **UPDATE (post-plan, both RESOLVED):** gap 1 (`VerifierImage` unwiring) fixed in commit `450a20e4`; gap 2 (verifier `estimated-cost` label) fixed in commit `65747947` (verifier Job now carries the label so `budget.RederiveReservations` restores its reservation after a manager restart — TASK-05/ESC-04). Both had a test added. Original gap descriptions preserved for the record:

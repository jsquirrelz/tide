...
- 🚧 **v1.0.1 — Orchestrator Trustworthiness + Telemetry Completion** (Phases 12–17) — Fix dogfood run-1 findings; make the orchestrator trustworthy enough to gate run 2 on; complete the merged telemetry foundation; close the milestone-audit tech-debt subset.

## Phases

### v1.0.1 — Orchestrator Trustworthiness + Telemetry Completion

**Milestone Goal:** Fix the dogfood run-1 findings (gate semantics run-killer, reject/resume recovery, image dispatch chain, billing halt, budget UX, paper cuts) and complete the merged telemetry foundation. Every fix carries a regression test reproducing the run-1 symptom. Phase 17 closes the in-scope tech-debt subset surfaced by the milestone audit.
...
- [x] **Phase 17: Tech Debt — Plan Label Backfill + Gate Hardening** - PlanReconciler project-label self-heal + reject-before-reporter-spawn + narrowed approve guard + non-fatal envelope-read (completed 2026-06-13)
...
### Phase 17: Tech Debt — Plan Label Backfill + Gate Hardening
...
(14 and 15 depend only on Phase 12 and can be planned in parallel once Phase 12 is complete; 17 closes the milestone-audit tech-debt subset)

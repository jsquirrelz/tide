...
2. **(RESOLVED: see 44-03-PLAN.md — interpolate between bracketing user-event timestamps, unconditional TimingSynthetic marker)** **Exact absolute span timing for a per-call LLM span, given `events.jsonl` has no in-band absolute timestamp for `message_start`/`message_stop`/`assistant` events**

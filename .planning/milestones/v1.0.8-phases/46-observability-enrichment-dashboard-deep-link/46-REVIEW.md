...
**Status:** fixed — `565daae`. Root cause: `synthesizePlannerSpan` now returns `emitted=false` when the minted SpanID is invalid or equals the parent's (the noop-propagation signature), so no handler ever persists a zero-hex `{Level}TraceSpanID`. Defense in depth: `PhoenixTraceLink` eligibility rejects all-zero spanIds for pre-upgrade CRs. Proving tests on both sides (`TestSynthesizePlannerSpanTracingDarkNotEmitted` under the real noop provider; all-zero spanId case in `node-panel-integration.test.tsx`). The ratio-dropped-root dead-link variant remains, covered by WR-02's documented limitation.
...
**Status:** fixed — `8df3cfd` (minimum fix per recommendation). The project-site comment now states the root-specific behavior honestly (ratio-gated root, same-reconcile threading, cross-reconcile default-true mislabel as bounded degradation); `docs/observability.md` scopes the ratio-drop → reporter-drop claim to the same-reconcile path with the retry exception named. No behavior change; inert at the 1.0 default.
...
**Status:** fixed — `2f1e26c` (documentation option per recommendation). `buildLevelEnrichment`'s doc comment now scopes the byte-identical guarantee to same-reconcile and names the accepted bounded drift (failure_halt/profile/gate values on a cross-reconcile spawn retry; session.id immune), matching the WR-02 doc fix symmetrically.
...
**Status:** fixed — `8e85b9e`. Test 8c (`TestParseFlagsEnrichmentTriple`, present + absent subtests per the Test 8b precedent) proves the triple parses onto `SessionID`/`MetadataJSON`/`TagsCSV`; the inline CSV split is extracted to `splitTags` and table-tested (`TestSplitTags`: empty, doubled comma, trailing comma, only-commas).

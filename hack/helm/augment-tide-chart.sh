...
ARGS53_BLOCK = """          {{- $verifyPosture := .Values.subagent.verify.posture | default "auto" }}
          ...
          {{- /* dig, not a direct .data.posture deref: a hand-recreated/patched
               ...
               hand-editing this object) would nil-pointer the render and brick
               every subsequent helm upgrade. dig degrades any missing layer to
               "" — same $verifyMarkerOn result for absent marker, empty marker,
